...
public class Java_Adv_Imp_Library {
    ...
    public static void matrixMultiplyInt(int[][] A, int[][] B, int size, String caseDescription, int runs) {
        ...
        try (FileWriter writer = new FileWriter(fileName)) { // Try catch for creation of file after every new test case
            for (int r = 0; r < runs; r++) { // Runs used to benchmark every 1, 5, 10, 20 times
                IntMatrix matrixA = new IntMatrix(A);
                IntMatrix matrixB = new IntMatrix(B);
                IntMatrix resultMatrix = matrixA.mmul(matrixB); // Matrix multiplication using jblas
                writer.write(resultMatrix.toString().replace(";", "\n")); // Writes results to file
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static void matrixMultiplyFloat(float[][] A, float[][] B, int size, String caseDescription, int runs) {
        ...
        try (FileWriter writer = new FileWriter(fileName)) { // Try catch for creation of file after every new test case
            for (int r = 0; r < runs; r++) { // Runs used to benchmark every 1, 5, 10, 20 times
                FloatMatrix matrixA = new FloatMatrix(A);
                FloatMatrix matrixB = new FloatMatrix(B);
                FloatMatrix resultMatrix = matrixA.mmul(matrixB); // Matrix multiplication using jblas
                writer.write(resultMatrix.toString().replace(";", "\n")); // Writes results to file
            }
        } catch (IOException e) {
            e.printStackTrace();
